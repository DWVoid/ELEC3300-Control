...
void LCD_BackLed_Control(FunctionalState enumState) {
    HAL_GPIO_WritePin(LCD_BK_PORT, LCD_BK_PIN, (enumState ? GPIO_PIN_RESET : GPIO_PIN_SET));
}
...
void LCD_FillColor(uint32_t usPoint, uint16_t usColor) {
    ...
    for (uint32_t i = 0; i < usPoint; i++) LCD_Write_Data(usColor);
}
...
uint16_t LCD_Read_PixelData(void) {
    LCD_Write_Cmd(0x2E);
    const uint16_t usDummy = LCD_Read_Data();    /*FIRST READ OUT DUMMY DATA*/
    const uint16_t usR = LCD_Read_Data();    /*READ OUT RED DATA  */
    const uint16_t usB = LCD_Read_Data();    /*READ OUT BLUE DATA*/
    const uint16_t usG = LCD_Read_Data();    /*READ OUT GREEN DATA*/
...
void LCD_DrawLine(uint16_t usC1, uint16_t usP1, uint16_t usC2, uint16_t usP2, uint16_t usColor) {
    int32_t lError_C = 0, lError_P = 0,  lDistance;
    int32_t lIncrease_C, lIncrease_P;

    int32_t lDelta_C = usC2 - usC1;
    int32_t lDelta_P = usP2 - usP1;

    uint16_t usC_Current = usC1;
    uint16_t usP_Current = usP1;
    ...
    for (uint16_t us = 0; us <= lDistance + 1; us++) {
...
void LCD_DrawString_Grid(uint16_t usC, uint16_t usP, const char *pStr) {
    LCD_DrawString(WIDTH_EN_CHAR * usC, HEIGHT_EN_CHAR * usP, pStr);
}
